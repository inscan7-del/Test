-- PHOENIX HUB 🐦‍🔥 | Main - Muscle Legends
...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/inscan7-del/Libary-/refs/heads/main/Library%20(1).txt", true))()

local window = library:AddWindow("PHOENIX HUB 🐦‍🔥 | Main - " .. Utils.greeting(), {
	title_bar = {Color3.fromRGB(138, 138, 138), Color3.fromRGB(255, 255, 255)},
	title_bar_transparency = 0.3,
	background = {Color3.fromRGB(119, 119, 119), Color3.fromRGB(200, 200, 200)},
...
Tabs.Info:Show()
Tabs.Info:AddLabel("PHOENIX HUB 🐦‍🔥").TextSize = 17


Tabs.Info:AddLabel("")
local wLabel = Tabs.Info:AddLabel("PHOENIX HUB 🐦‍🔥 [V1.0.0]")
Tabs.Info:AddLabel("تم الصنع من يوسف و فهد").TextSize = 20
Tabs.Info:AddLabel("إذا واجهتك أي مشاكل يرجى التواصل من هنا").TextSize = 16
Tabs.Info:AddButton("نسخ رابط التواصل", function()
    if setclipboard then
        setclipboard("https://t.me/c/3161086135/590921")
        game.StarterGui:SetCore("SendNotification", {
            Title = "تم النسخ",
            Text = "تم نسخ رابط التواصل بنجاح.",
            Duration = 3
        })
    else
        game.StarterGui:SetCore("SendNotification", {
            Title = "خطأ",
            Text = "النسخ غير مدعوم.",
            Duration = 3
        })
    end
end)
...
Tabs.Inventory:AddSwitch("Eat All Eggs", function(state) EggEaterData.Running = state end)
...
local petDropdown = Tabs.Inventory:AddDropdown("Choose Pet", function(text)
...
Tabs.Inventory:AddButton("🛒 Buy Selected Pet", function()
    buySelectedPet()
end)

Tabs.Inventory:AddSwitch("⚡ Auto Buy Selected Pet", function(bool)
...
local gymLocations = {
	...
	{name = "Industrial Gym", pos = "AUTO_FIND"}
}

local function findIndustrialGymPad()
	local roots = {}
	local pads = workspace:FindFirstChild("areaTeleportParts")
	if pads then table.insert(roots, pads) end
		...
		for _, obj in ipairs(root:GetDescendants()) do
			if obj:IsA("BasePart") then
				...
				for _ = 1, 5 do
					...
					local n = tostring(cur.Name):lower():gsub("[%s_%-]", "")
					if n:find("industrial") then
						return obj
					end
					cur = cur.Parent
				end
...
for _, gym in ipairs(gymLocations) do
	Tabs.Teleport:AddButton(gym.name, function()
		local char = PlayerData.Player.Character or PlayerData.Player.CharacterAdded:Wait()
		local hrp = char:FindFirstChild("HumanoidRootPart")
		if not hrp then return end

		if gym.pos == "AUTO_FIND" then
			local target = findIndustrialGymPad()

			-- Prefer the game's area travel remote when it exists.
			local rEvents = Services.ReplicatedStorage:FindFirstChild("rEvents")
			local areaRemote = rEvents and rEvents:FindFirstChild("areaTravelRemote")
			local moved = false

			if areaRemote then
				for _, args in ipairs({
					{"travelToArea", "Industrial Gym"},
					{"travelToArea", "Industrial"},
					{"Industrial Gym"},
				}) do
					local ok = pcall(function()
						areaRemote:InvokeServer(unpack(args))
					end)
					if ok then
						task.wait(0.5)
						moved = true
			...
			if target then
				for _ = 1, 12 do
					...
					if currentHrp then
						currentHrp.CFrame = target.CFrame * CFrame.new(0, 6, 0)
			...
			if not moved then
				warn("PHOENIX HUB: Industrial Gym pad was not found.")
			end
		else
			hrp.CFrame = gym.pos
		end
